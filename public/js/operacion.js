...
function setClaveOk(id_viaje,clave){
  var date = $('#fecha_horax').val();
	...
	var datex = Base64.encode(date);
  var automan = $('#automan').val();

	$(document).ready(function() {
		$.ajax({
			url: 'operacion/setClaveOk/' + id_viaje + '/' + clave + '/' + datex + '/' +  automan,
			beforeSend: function( xhr ) {
				$('#myModal').modal('hide');
				$('#myModal2').modal('hide');
		         	$('#initpreloader').fadeIn('slow');
...
function nueva_incidencia(id_viaje){
	$(document).ready(function() {
		$.ajax({
			url: 'operacion/nueva_incidencia/' + id_viaje,
			dataType: 'html',
			success: function(resp_success){
				var modal =  resp_success;
				$(modal).modal().on('shown.bs.modal',function(){
...
function nueva_incidencia_do(){
	var msj_error="";
	if( $('#cat_incidencias').get(0).value == "" )	msj_error+='Seleccione la incidencia del catálogo.<br />';
	if( $('#id_viaje').get(0).value == "" )		msj_error+='Falta id_viaje, no deberia ocurrir esto.<br />';
	...
	$(document).ready(function() {
		$.ajax({
			url: 'operacion/nueva_incidencia_do',
			type: 'POST',
			data: $("#nueva_incidencia").serialize(),
			dataType: 'json',
			success: function(resp_success){
				if (resp_success['resp'] == true) {
...
function close_nueva_incidencia_form(){
	$("#add_field").css({ display: "none" });
	$("#footer_main").css({ display: "" });
}
function eliminar_incidencia(id_incidencia){
	$(document).ready(function() {
		$.ajax({
			url: 'operacion/eliminar_incidencia/' + id_incidencia,
			dataType: 'json',
			success: function(resp_success){
				if (resp_success['resp'] == true) {
					$('#nuevaIncidencia').DataTable().ajax.reload();
				}else{
					alerta('Alerta!','Error de conectividad de red OPRN-106');
				}
			},
			error: function(respuesta){ alerta('Alerta!','Error de conectividad de red OPRN-107');}
		});
	} );
}
...
function modificar_revision(id_viaje){
	$(document).ready(function() {
		$.ajax({
			url: 'operacion/modificar_revision/' + id_viaje,
			dataType: 'html',
				success: function(resp_success){
					var modal =  resp_success;
					$(modal).modal().on('shown.bs.modal',function(){
			...
			error: function(respuesta){ alerta('Alerta!','Error de conectividad de red OPRN-124');}
		});
	} );
}
function modificar_revision_set(id_viaje){

		$.ajax({
			url: url_app + 'operacion/modificar_revision_set/' + id_viaje,
			type: 'POST',
			data: $("#setear_revision_viaje").serialize(),
			dataType: 'json',
			success: function(resp_success){
				if (resp_success['resp'] == true) {
						$('#myModal').modal('hide');
					...
					alerta('Alerta!', 'Error de conectividad de red OPRN-125');
				}
			},
			error: function(respuesta){ alerta('Alerta!','Error de conectividad de red OPRN-126');}
		});
