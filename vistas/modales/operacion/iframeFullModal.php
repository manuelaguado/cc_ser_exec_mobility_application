...
<html>
    ...
    <script>
		function initMap() {
			$('#origen_referencia', window.parent.document).val('');
			...
			google.maps.event.addListener(map, "dblclick", function(event) {
				...
				$( '#geocoordenadas_origen', window.parent.document ).val(lat+','+lng);
							...
							for(var k in data.results[0].address_components[i].types){
								...
								switch (data.results[0].address_components[i].types[k]) {
									...
									case 'administrative_area_level_3':
										...
										break;
									case 'route':
										$('#origen_calle', window.parent.document).val(data.results[0].address_components[i].long_name);
										$('#origen_hide_ok', window.parent.document).removeClass('hide');
										break;
									case 'street_number':
										$('#origen_num_ext', window.parent.document).val(data.results[0].address_components[i].long_name);
										$('#origen_hide_ok', window.parent.document).removeClass('hide');
										break;
									case 'postal_code':
