...
class IngresosoperadorModel
{
    ...
    function pausadosGroup($array){
           ...
           v.id_viaje ASC
           ';
           $render_table = new accionespausadosGroup;
           return json_encode(
                  $render_table->complex( $array, $this->dbt, $table, $primaryKey, $columns, null, $where, $inner, null, $orden )
           );
    }
    function viajes_pausados($array,$id_operador){
           ini_set('memory_limit', '256M');
           $table = 'vi_viaje AS v';
           $primaryKey = 'id_viaje';
           $columns = array(
           ...
           $where = "
                  o.id_operador = $id_operador
           AND vs.cat_status_statics = 222
           AND v.cat_status_viaje = 251
           ";
           $orden = '
           ORDER BY
           	id DESC
           ';
           $render_table = new accionesviajes_pausados;
           return json_encode(
                  $render_table->complex( $array, $this->dbt, $table, $primaryKey, $columns, null, $where, $inner, null, $orden )
           );
    }
...
class accionespausadosGroup extends SSP{
	static function data_output ( $columns, $data, $db )
	{
		$out = array();
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
