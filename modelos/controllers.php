...
class ControllersModel
{
	...
	public function eliminar_metodo($id_metodo){
		...
		$pquery = $this->db->prepare("DELETE FROM fw_permisos WHERE id_metodo = $id_metodo");
		$pquery->execute();
        
		$sql = "DELETE FROM fw_metodos WHERE id_metodo = :id_metodo";
        $query = $this->db->prepare($sql);
        $query_resp = $query->execute(array(':id_metodo' => $id_metodo));
		if($query_resp){
